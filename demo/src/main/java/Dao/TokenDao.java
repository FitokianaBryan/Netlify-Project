...
public class TokenDao {
    ...
    public String insertTokenAdmin(String idadmin){

        try {
            String token = createToken(idadmin);
            Date creation = Date.valueOf(LocalDate.now());
            Date expiration = Date.valueOf(LocalDate.now().plusDays(3));
            String role = "Admin";
            String requete = "insert into tokenAdmin(idadmin,token,datecreation,dateexpiration,role) values ('"+idadmin+"','"+token+"','"+creation+"','"+expiration+"','"+role+"')";
    ...
    public void deleteTokenAdmin(String token,String idAdmin){
        Connexion conn = null;
        try {
